...
const possibleEvents = ['SNS', 'S3', 'SQS'];
...
class AwsListenerTest {
	...
	validateRule(rule) {
		...
		if(typeof rule.event === 'string' && !possibleEvents.includes(rule.event))
			throw new Error(`Event must be one of this options: ${possibleEvents.join(',')}`);
